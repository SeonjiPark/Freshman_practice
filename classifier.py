...
class Classifier(object):
    ...
    def build(self, x, reuse=None):
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            Conv1 = tf.layers.conv2d(x_reshape, 32, [3, 3], activation=tf.nn.relu)
            Conv1 = tf.layers.max_pooling2d(Conv1, 2, 2)

            Conv2 = tf.layers.conv2d(Conv1, 64, [3, 3], activation=tf.nn.relu)
            Conv2 = tf.layers.max_pooling2d(Conv2, 2, 2)

            Conv3 = tf.layers.conv2d(Conv2, 64, [3, 3], activation=tf.nn.relu)

            Conv4 = tf.layers.conv2d(Conv3, 64, [3, 3], activation=tf.nn.relu)

            Conv4_flat = tf.contrib.layers.flatten(Conv4)

            Fc1 = tf.layers.dense(Conv4_flat, 256, activation=tf.nn.relu)
            Fc1 = tf.nn.dropout(Fc1, 0.5)

            logit = tf.layers.dense(Fc1, 10, activation='softmax')
    ...
    def test(self):
        # load data
        ts_img, ts_lab = read_tfrecord(self.args.test_datadir, self.args.batch, None)
